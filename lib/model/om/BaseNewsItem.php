...
/**
 * Base class that represents a row from the 'news_item' table.
 *
 * 
 *
...
abstract class BaseNewsItem extends BaseObject  implements Persistent {
  ...
  const PEER = 'NewsItemPeer';
	 ...
	 * Note: this column has a database default value of: 'PUBLISHED'
	 * @var        string
	 */
	protected $state;
	...
	/**
	 * @var        array NewsItemI18n[] Collection to store aggregation of NewsItemI18n objects.
	 */
	protected $collNewsItemI18ns;

	/**
	 * @var        Criteria The criteria used to select the current contents of collNewsItemI18ns.
	 */
	private $lastNewsItemI18nCriteria = null;
  ...
  protected $culture;
	...
	/**
	 * Initializes internal state of BaseNewsItem object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     NewsItem The current object (for fluent API support)
	 */
	...
	public function hasOnlyDefaultValues()
	{
			...
			if (array_diff($this->modifiedColumns, array(NewsItemPeer::STATE))) {
				return false;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->photo_filename = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			...
			return $startcol + 6; // 6 = NewsItemPeer::NUM_COLUMNS - NewsItemPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = NewsItemPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->collNewsItemI18ns = null;
			$this->lastNewsItemI18nCriteria = null;
    ...
    foreach (sfMixer::getCallables('BaseNewsItem:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseNewsItem:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseNewsItem:save:pre') as $callable)
    {
    ...
    if ($this->isModified() && !$this->isColumnModified(NewsItemPeer::UPDATED_AT))
    {
      $this->setUpdatedAt(time());
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseNewsItem:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			NewsItemPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of NewsItem (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     NewsItem Clone of current object.
	 * @throws     PropelException
	 */
	...
	/**
	 * Clears out the collNewsItemI18ns collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 * @return     void
	 * @see        addNewsItemI18ns()
	 */
	public function clearNewsItemI18ns()
	{
		$this->collNewsItemI18ns = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collNewsItemI18ns collection (array).
	 *
	 * By default this just sets the collNewsItemI18ns collection to an empty array (like clearcollNewsItemI18ns());
	...
	/**
	 * Gets an array of NewsItemI18n objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this NewsItem has previously been saved, it will retrieve
	 * related NewsItemI18ns from storage. If this NewsItem is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array NewsItemI18n[]
	 * @throws     PropelException
	 */
	public function getNewsItemI18ns($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(NewsItemPeer::DATABASE_NAME);
		}
	...
	/**
	 * Returns the number of related NewsItemI18n objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related NewsItemI18n objects.
	 * @throws     PropelException
	 */
	public function countNewsItemI18ns(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(NewsItemPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collNewsItemI18ns === null) {
			if ($this->isNew()) {
				...
				$count = NewsItemI18nPeer::doCount($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastNewsItemI18nCriteria) || !$this->lastNewsItemI18nCriteria->equals($criteria)) {
					...
					$count = count($this->collNewsItemI18ns);
				}
			} else {
				$count = count($this->collNewsItemI18ns);
			}
	...
	/**
	 * Method called to associate a NewsItemI18n object to this object
	 * through the NewsItemI18n foreign key attribute.
	 *
	 * @param      NewsItemI18n $l NewsItemI18n
	 * @return     void
	 * @throws     PropelException
	 */
	...
	{
		if ($this->collNewsItemI18ns === null) {
			$this->initNewsItemI18ns();
		}
		if (!in_array($l, $this->collNewsItemI18ns, true)) { // only add it if the **same** object is not already associated
			array_push($this->collNewsItemI18ns, $l);
			$l->setNewsItem($this);
		}
	}
	...
	{
		if ($deep) {
			if ($this->collNewsItemI18ns) {
				foreach ((array) $this->collNewsItemI18ns as $o) {
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseNewsItem:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseNewsItem::%s', $method));
    }
