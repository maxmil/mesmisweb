...
abstract class BaseProduct extends BaseObject  implements Persistent {
  ...
  const PEER = 'ProductPeer';
	...
	public function setDescrip($v)
	{
		...
		if ($this->descrip !== $v) {
			$this->descrip = $v;
			$this->modifiedColumns[] = ProductPeer::DESCRIP;
		}
	...
	public function setPhotoFilename($v)
	{
		...
		if ($this->photo_filename !== $v) {
			...
			$this->modifiedColumns[] = ProductPeer::PHOTO_FILENAME;
		}
	...
	public function setAttachFilename($v)
	{
		...
		if ($this->attach_filename !== $v) {
			...
			$this->modifiedColumns[] = ProductPeer::ATTACH_FILENAME;
		}
	...
	public function hasOnlyDefaultValues()
	{
			...
			if (array_diff($this->modifiedColumns, array(ProductPeer::STATE))) {
				return false;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->descrip = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->photo_filename = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->attach_filename = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			...
			$keys[2] => $this->getDescrip(),
			$keys[3] => $this->getPhotoFilename(),
			$keys[4] => $this->getAttachFilename(),
			$keys[5] => $this->getUrl(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[2], $arr)) $this->setDescrip($arr[$keys[2]]);
		if (array_key_exists($keys[3], $arr)) $this->setPhotoFilename($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setAttachFilename($arr[$keys[4]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ProductPeer::DESCRIP)) $criteria->add(ProductPeer::DESCRIP, $this->descrip);
		if ($this->isColumnModified(ProductPeer::PHOTO_FILENAME)) $criteria->add(ProductPeer::PHOTO_FILENAME, $this->photo_filename);
		if ($this->isColumnModified(ProductPeer::ATTACH_FILENAME)) $criteria->add(ProductPeer::ATTACH_FILENAME, $this->attach_filename);
	 ...
	 * related Downloads from storage. If this Product is new, it will return
		...
		if ($this->collDownloads === null) {
			if ($this->isNew()) {
			   $this->collDownloads = array();
				...
				$criteria->add(DownloadPeer::PRODUCT_ID, $this->id);
	 ...
	 */
	public function getDownloadsJoinUser($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(ProductPeer::DATABASE_NAME);
		}
		...
		if ($this->collDownloads === null) {
			if ($this->isNew()) {
				...
				$this->collDownloads = DownloadPeer::doSelectJoinUser($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
